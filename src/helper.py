...
def conv_weight(counts, mask, SHAPE):
    weights = np.zeros_like(counts)
    for idx, slic in enumerate(counts):
        if slic.any(): # we ignore slices which don't have any slices
            slic += mask[idx] # but add the mask as one weight
            indices = np.array(np.where(slic > 0)).T
            a, b = indices.min(axis=0)
            c, d = indices.max(axis=0)
            filt = np.ones((SHAPE[1], SHAPE[2]))
            weights[idx, a:c, b:d] = scipy.signal.convolve2d(slic[a:c, b:d], filt, mode='same')   
